...
| **Playstyle** | A character, not a strength: among lines the engine already called equal, *Attacking* takes the forcing move (check, capture, promotion) and *Quiet* takes the calm one, inside a **35cp** tolerance so the style never plays a worse move. On the human nets, which price moves in probability rather than centipawns, the tolerance is **two thirds of the top move's probability** instead - so it can never reach for something a person would rarely play. Needs more than one line (**Multi Lines ≥ 2**), so it does nothing on a single-line cloud or remote engine. *Balanced* (the default) is the engine's own pick, and book and tablebase moves still outrank it. |
