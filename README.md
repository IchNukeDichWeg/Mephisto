...
![Version](https://img.shields.io/badge/version-3.1.303-3fb950)
...
| **Maia-2** | One human model that takes **both** ratings - yours and your opponent's - so the same position is answered differently by a 1200 facing a 2000 than by a 2000 facing a 1200. Two -/+ steppers in 50s, both live. Verified 40/40 against maia2's own inference. |
...
| **Playstyle** | A character, not a strength - ten of them, chosen among lines the engine already called equal. *Attacking* takes the forcing move (check, capture, promotion), *Quiet* the calm one, *Greedy* the most material, *Space* the move that gains ground (pawns counted double), *Sacrifice* the one that offers material the opponent may take, *Safe* the one that leaves nothing hanging, *Drawish* the line closest to equality, *Ultra attacking* throws pieces at the king, and *Disrespect* is the rude one - a second queen while one is already on the board, a sacrifice made while already winning (**+6 before the move, +2 after**), an opening on the rim, the king strolling toward theirs. Never at a cost: **35cp**, or on a human net **two thirds of the top move's probability**, since those nets price moves in probability and not centipawns - which is why a *Sacrifice* here is one the engine already rates as sound rather than a blunder wearing a bow. Needs **Multi Lines ≥ 2**, and the control hides itself where it cannot apply (cloud and remote engines, four-player chess, Fairy variants). *Balanced* (the default) is the engine's own pick, and book and tablebase moves still outrank it. |
...
- [x] **Grind Mode confirmed on Chess.com** (v3.1.268) - the click-through v3.1.267 could not verify is
  confirmed working in a real game. Chess.com renders that button two ways and both are handled: the
...
- [x] **Grind Mode on Chess.com too** (v3.1.267) - the game-over modal there gives its buttons nothing but
  ...
  chess.com games (a bot game I played and resigned, and the button HTML from a finished online game), and the
